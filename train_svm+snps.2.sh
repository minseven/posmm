...
sort $DIR/svm+snps/l$l/snps.controls.tmp | uniq -c | gawk '{print $1 "\t" $2}' | sed '1d' > $DIR/svm+snps/l$l/snps+count.$i.controls.tmp

~/bin/svm+snps_pvalue.py $DIR/svm+snps/l$l/snps+count.$i.cases.tmp $DIR/svm+snps/l$l/snps+count.$i.controls.tmp $NCASE $NCONTROL $DIR/svm+snps/l$l/${p1}/snps.$i.tmp1 ${p2}
~/bin/svm+snps_pvalue.py $DIR/svm+snps/l$l/snps+count.$i.controls.tmp $DIR/svm+snps/l$l/snps+count.$i.cases.tmp $NCASE $NCONTROL $DIR/svm+snps/l$l/${p1}/snps.$i.tmp2 ${p2}
...
rm $DIR/svm+snps/l$l/${p1}/*tmp*

for p1 in 10 50 100 500 1000;
do
        mkdir -p $DIR/svm+snps/l$l/${p1}
        sort -k 2 -g $DIR/svm+snps/l$l/snps.$i.list | head -n ${p1} | gawk '{print $1}' > $DIR/svm+snps/l$l/${p1}/snps.$i.list
done
