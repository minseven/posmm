...
do
	a=`(gawk '{print NF}' $DIR/svm+snps/l$l/${p1}/snps.$name.matrix.tmp)`
        if [ $a -eq 0 ];
        then
                echo "1" > $DIR/svm+snps/l$l/${p1}/snps.$name.matrix.tmp
        fi
	cat $DIR/svm+snps/l$l/${p1}/snps.$name.matrix.tmp >> $DIR/svm+snps/l$l/${p1}/test_snps.matrix.tmp
done

~/bin/FIP_to_libsvm2.py $DIR/$TEST_LIST $DIR/svm+snps/l$l/${p1}/test_snps.matrix.tmp $DIR/svm+snps/l$l/${p1}/test_snps.libsvm $CASE
~/bin/FIP_to_libsvm2.py $DIR/$TRAIN_LIST $DIR/svm+snps/l$l/${p1}/snps.matrix.tmp $DIR/svm+snps/l$l/${p1}/train_snps.libsvm $CASE
